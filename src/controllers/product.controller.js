...
export const updateProductCtrl = async (req, res) => {
  try {
    ...
    if (
      ...
      updateObj?.thumbnail?.key &&
      updateObj?.thumbnail?.key !== product?.thumbnail?.key
    ) {
      try {
    ...
    const oldImgKeys = Array.isArray(product?.images)
      ? product?.images?.map((img) => img?.key)
      : [];
    const newImgKeys = Array.isArray(updateObj?.images)
      ? updateObj?.images?.map((img) => img?.key)
      : [];

    if (oldImgKeys?.length > 0) {
      const deletableKeys = oldImgKeys?.filter(
        (oik) => !newImgKeys?.includes(oik)
      );
...
export const getManyProductsCtrl = async (req, res, next) => {
  try {
    ...
    if (minPrice || maxPrice) {
      filters.price = {};
      if (Number.isFinite(parseInt(minPrice)))
        filters.price.$gte = parseInt(minPrice);
      if (Number.isFinite(parseInt(maxPrice)))
...
export const getAllProductsCtrl = async (req, res, next) => {
  try {
    ...
    if (stock === "in-stock") {
      ...
      const threshold = 5;
      advancedFilters.$expr = {
        $and: [
          { $gt: [{ $sum: "$variantItems.stock" }, 0] },
          { $lt: [{ $sum: "$variantItems.stock" }, threshold] },
        ],
      };
    }
    ...
    let sort = { name: 1 };
    if (
      ["name", "price", "inventory"]?.includes(sortBy) &&
    ...
    if (isValidObjectId(category)) {
      ...
      const productIds =
        cats?.flatMap((cat) => cat?.productIds?.map((item) => item?._id)) ?? [];

      filters._id = { $in: productIds };
    }

    const paginatedResult = await getAllProducts({
      filters,
      advancedFilters,
      sort,
      page,
...
export const updateVariationCtrl = async (req, res) => {
  try {
    ...
    if (
      ...
      existingVariation[0]?._id?.toString() !== variationId
    ) {
      return res.status(400).json({
        success: false,
        message: "Variation already exists",
...
export const updateOptionCtrl = async (req, res) => {
  try {
    ...
    if (
      ...
      existingOption[0]?._id?.toString() !== optionId
    ) {
      return res.status(400).json({
        success: false,
        message: "Option already exists",
