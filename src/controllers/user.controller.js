...
export const registerUserCtrl = async (req, res) => {
    try {
        ...
        // ✅ Allow two modes: password or OTP
        if (!['email', 'mobile', 'password'].includes(credType)) {
            return res.status(400).json({
                success: false,
                message: 'Invalid credential type',
                data: null,
        ...
        if (credType !== 'password') {
            ...
            // if(validOtp){
            //     const verifiedOtp = await verifyOTP(validOtp?._id)
            // }

            if (validOtp) {
                const isOTPVerified = await OTPVerificationStatus(validOtp?._id);
                console.log(isOTPVerified ,"otp")
                if (!isOTPVerified) {
                    return res.status(400).json({
                        success: false,
                        message: 'OTP is not verified',
        ...
        // ✅ Only require password if password flow
        if (password?.trim()) {
            const hashedPassword = await hashPassword(password);
            createObj.password = hashedPassword;
